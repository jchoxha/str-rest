...
import mountainViewDeckImg from '../assets/mountain_view_deck.png'
import pourOverImg from '../assets/pour_over.png'
import riversideBreweryImg from '../assets/riverside_brewery.png'
import tinyHomeImg from '../assets/tiny_home.png'
import tinyHomeKitchenImg from '../assets/tiny_home_kitchen.png'
import velvetPillowImg from '../assets/velvet_pillow.png'

const assetMap = {
  'hero.png': heroImg,
  'host_portrait.png': hostPortraitImg,
  'mountain_view_deck.png': mountainViewDeckImg,
  'pour_over.png': pourOverImg,
  'riverside_brewery.png': riversideBreweryImg,
  'tiny_home.png': tinyHomeImg,
  'tiny_home_kitchen.png': tinyHomeKitchenImg,
  'velvet_pillow.png': velvetPillowImg,
}

function img(name) {
  return assetMap[name] || name
}
...
export function findDemoProperty(slug) {
  if (!slug) return null
  const direct = DEMO_PROPERTIES[slug.toLowerCase()]
  if (direct) return direct
  const norm = normalizeSlug(slug)
  return Object.values(DEMO_PROPERTIES).find(p => normalizeSlug(p.slug) === norm) || null
}

export function getDemoProperty(slug) {
  const p = findDemoProperty(slug)
  if (!p) return null
  // Return public version (stripping home-details)
  const publicContent = { ...p.content }
  delete publicContent['home-details']
  return {
    id: p.id,
    slug: p.slug,
    name: p.name,
    ...
    hostNames: p.hostNames || p.host_names,
    layouts: p.layouts,
...
export function unlockDemoProperty(slug, code) {
  const p = findDemoProperty(slug)
  if (!p) return null
  if (p.access_code && p.access_code.toLowerCase() === String(code).trim().toLowerCase()) {
    return {
      id: p.id,
      slug: p.slug,
      name: p.name,
